...
public interface RolePremissionMapper extends BaseMapper<RolePremission> {
    ...
    @Select("SELECT premissions.* FROM role,premissions,role_pre WHERE role.name=#{role} AND role_pre.role_id = role.name AND role_pre.premission_id = premissions.id;")
    Set<Premission> getAllPreByRole(String role);
}
